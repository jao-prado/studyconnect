...
public class PerfilAprendizado {
    ...
    @Column(length = 30)
    private String objetivo;       // ENEM, VESTIBULAR, REFORCO, FACULDADE

    @Column(length = 30)
    private String nivel;          // BASICO, INTERMEDIARIO, AVANCADO

    @Column(name = "horas_semana")
    private Integer horasSemana;   // 1, 3, 5, 10+

    @Column(name = "meta_semanal")
    private Integer metaSemanal;   // número de aulas por semana

    @Column(length = 20)
    private String ritmo;          // INTENSO, MODERADO, LEVE

    @Column(length = 500)
    private String interesses;     // CSV: "Matemática,Física"

    @Column(length = 500)
    private String dificuldades;   // CSV: "Química,História"
